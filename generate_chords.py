...
lowest_note = midi_manipulation.lowerBound # index of the lowest note on the piano roll
highest_note = midi_manipulation.upperBound # index of the highest note on the piano roll
note_range = highest_note-lowest_note # note range
...
def gibbs_sample(k):
    ...
    # This stops tensorflow from propagating gradients back through the gibbs step
    x_sample = tf.stop_gradient(x_sample)
    return x_sample

### Training Update Code
# Contrastive divergence algorithm.
# Get the samples of x and h from the probability distribution
...
# Uupdating the values of W, bh, and bv, based on the difference between the samples drawn and the original values
...
with tf.Session() as sess:
    ...
    sess.run(init)
    # Running through all of the training data num_epochs times
    for epoch in tqdm(range(num_epochs)):
        for song in songs:
            # The songs are stored in a time x notes format. The size of each song is timesteps_in_song x 2*note_range
            # Reshaping the songs so that each training example is a vector with num_timesteps x 2*note_range elements
            song = np.array(song)
            song = song[:int(np.floor(song.shape[0]//num_timesteps)*num_timesteps)]
            song = np.reshape(song, [song.shape[0]//num_timesteps, song.shape[1]*num_timesteps])
            # Training the RBM on batch_size examples at a time
            for i in range(1, len(song), batch_size):
                tr_x = song[i:i+batch_size]
                sess.run(updt, feed_dict={x: tr_x})

    # Running a gibbs chain where the visible nodes are initialized to 0
    sample = gibbs_sample(1).eval(session=sess, feed_dict={x: np.zeros((50, n_visible))})
    for i in range(sample.shape[0]):
        if not any(sample[i,:]):
            continue
        # Reshaping the vector to be time x notes, and then saving the vector as a midi file
